...
@push('styles')
    <style>
        ...
        *::after {
        ...
        .idx-table thead th:first-child {
            padding-left: 24px;
        }

        .idx-table thead th:last-child {
            padding-right: 24px;
            text-align: right;
        }
        ...
        .active .idx-badge-dot {
            background: var(--green);
        }

        .inactive .idx-badge-dot {
            background: var(--amber);
        }

        .closed .idx-badge-dot {
            background: var(--red);
        }
        ...
        @media(max-width:900px) {
            .idx-table thead th:nth-child(n+4):not(:last-child) {
                display: none;
            }

            .idx-table td:nth-child(n+4):not(:last-child) {
                display: none;
            }
        }
...
@section('content')
    <div class="idx-wrap">
                ...
                <div class="idx-hero-left">
                    ...
                    <div class="idx-stats">
                        @php
                            ...
                            $activeJobs = isset($jobs)
                                ? (is_object($jobs) && method_exists($jobs, 'getCollection')
                                    ? $jobs->getCollection()
                                    : collect($jobs)
                                )
                                    ->where('status', '1')
                                    ->count()
                                : 0;
                            $totalApps = isset($jobs)
                                ? (is_object($jobs) && method_exists($jobs, 'getCollection')
                                    ? $jobs->getCollection()
                                    : collect($jobs)
                                )->sum(fn($j) => $j->applications_count ?? ($j->applicants_count ?? 0))
                                : 0;
                        @endphp
                        <div class="idx-stat">
                        ...
                        <div class="idx-stat">
                            <div class="idx-stat-icon" style="background:rgba(74,222,128,.2);"><i
                        ...
                        <div class="idx-stat">
                            <div class="idx-stat-icon" style="background:rgba(251,191,36,.2);"><i
        ...
        @if (session('success'))
            <div
                style="display:flex;align-items:center;gap:12px;background:var(--green-lt);border:1.5px solid var(--green-mid);border-radius:var(--r-sm);padding:14px 20px;font-family:var(--fb);font-size:.88rem;color:#065f46;margin-bottom:16px;">
                <i class="fa-solid fa-circle-check" style="color:var(--green);font-size:1rem;"></i> {{ session('success') }}
            </div>
        @endif
        @if (session('error'))
            <div
                style="display:flex;align-items:center;gap:12px;background:var(--red-lt);border:1.5px solid var(--red-mid);border-radius:var(--r-sm);padding:14px 20px;font-family:var(--fb);font-size:.88rem;color:#b91c1c;margin-bottom:16px;">
                <i class="fa-solid fa-triangle-exclamation" style="color:var(--red);font-size:1rem;"></i>
                {{ session('error') }}
            </div>
        @endif
        ...
        <div class="idx-filters">
            <div class="idx-search-wrap">
                ...
                <input type="text" id="jobSearch" class="idx-search"
                    placeholder="Search by job title, location or category..." oninput="filterJobs()">
            </div>
        ...
        <div class="idx-table-card">
            <div class="idx-table-header">
                ...
                <div
                    style="font-family:var(--fb);font-size:.78rem;color:var(--ink3);display:flex;align-items:center;gap:6px;">
            ...
            @if (isset($jobs) && $jobs->count() > 0)
                <div style="overflow-x:auto;">
                    <table class="idx-table" id="jobsTable">
                        ...
                        <tbody>
                            @foreach ($jobs as $job)
                                <tr class="job-row" data-title="{{ strtolower($job->title) }}"
                                            ...
                                            <div class="idx-job-ico">
                                                @php
                                                    $catIcons = [
                                                        ...
                                                        'Technical & Trade' => 'fa-wrench',
                                                        'Sales & Marketing' => 'fa-chart-line',
                                                        'Office & Admin' => 'fa-building',
                                                        'Driver & Logistics' => 'fa-truck',
                                            ...
                                            <div>
                                                ...
                                                <div class="idx-job-meta">
                                                    @if ($job->job_type ?? false)
                                                        <span class="idx-meta-pill"><i class="fa-solid fa-clock"></i>
                                                            {{ $job->job_type }}</span>
                                                    @endif
                                                    @if ($job->experience_required ?? false)
                                                        <span class="idx-meta-pill"><i class="fa-solid fa-briefcase"></i>
                                                            {{ $job->experience_required }}</span>
                                                    @endif
                                                    @if ($job->salary_range ?? false)
                                                        <span class="idx-meta-pill"><i
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ route('employer.candidates', $job->id) }}" class="idx-app-count"
                                    ...
                                    <td>
                                        <div
                                            style="font-family:var(--fh);font-size:.78rem;font-weight:700;color:var(--ink2);">
                                            {{ $job->category ?? '—' }}</div>
                                    </td>
                                    ...
                                    <td>
                                        <div
                                            style="display:flex;align-items:center;gap:5px;font-family:var(--fb);font-size:.82rem;color:var(--ink3);">
                                            <i class="fa-solid fa-location-dot"
                                                style="font-size:.68rem;color:var(--ink4);"></i>
                                            {{ $job->city ?? '' }}@if ($job->district ?? false)
                                                , {{ $job->district }}
                                            @endif
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <div class="idx-actions">
                                            <a href="{{ route('employer.jobs.show', $job->id) }}" class="idx-btn view"
                                                title="View Job" target="_blank">
                                                <i class="fa-solid fa-eye"></i>
                                            </a>
                                            <a href="{{ route('employer.jobs.edit', $job->id) }}" class="idx-btn edit"
                                                title="Edit Job">
                                                <i class="fa-solid fa-pen"></i>
                                            </a>
                                            @if ($appCount > 0)
                                                <a href="{{ route('employer.candidates.show', $job->id) }}"
                                                    class="idx-btn apps" title="View Applicants">
                                                    <i class="fa-solid fa-users"></i>
                                                </a>
                                            @endif

                                            <button class="idx-btn tog toggleJobBtn"
                ...
                <div id="noResultsRow" style="display:none;padding:60px 24px;text-align:center;color:var(--ink4);">
                    <i class="fa-solid fa-magnifying-glass"
                        style="font-size:2rem;opacity:.2;display:block;margin-bottom:14px;"></i>
                    <p style="font-family:var(--fb);font-size:.9rem;">No jobs match your search / filter. <button
                            onclick="resetFilters()"
                            style="background:none;border:none;color:var(--blue);cursor:pointer;font-family:var(--fh);font-weight:700;">Reset
                            filters</button></p>
                </div>
                ...
                @if (method_exists($jobs, 'links'))
                    <div class="idx-pagination">
                        ...
                        <div class="idx-pag-links">
                            ...
                            @else
                                <a href="{{ $jobs->previousPageUrl() }}" class="idx-pag-btn"><i
                                        class="fa-solid fa-chevron-left"></i></a>
                            @endif
                            @foreach ($jobs->getUrlRange(max(1, $jobs->currentPage() - 2), min($jobs->lastPage(), $jobs->currentPage() + 2)) as $page => $url)
                                <a href="{{ $url }}"
                                    class="idx-pag-btn {{ $page === $jobs->currentPage() ? 'active' : '' }}">{{ $page }}</a>
                            @endforeach
                            @if ($jobs->hasMorePages())
                                <a href="{{ $jobs->nextPageUrl() }}" class="idx-pag-btn"><i
            ...
            @else
                ...
                <div class="idx-empty">
                    ...
                    <p>You haven't posted any jobs. Create your first listing to start attracting the best talent across
                        Tamil Nadu.</p>
                    <a href="{{ route('employer.jobs.create') }}" class="btn-post-job"
    ...
    <script>
        ...
        function filterJobs() {
            ...
            rows.forEach(function(row) {
                ...
                var match = true;
                if (q && !title.includes(q) && !location.includes(q) && !category.toLowerCase().includes(q)) match =
                    false;
        ...
        function showToast(msg, type) {
            ...
            t.innerHTML = '<i class="fa-solid ' + (type === 'success' ? 'fa-circle-check' : 'fa-circle-exclamation') +
                '"></i> ' + msg;
