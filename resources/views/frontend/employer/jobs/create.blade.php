...
.pj-hero-inner { position: relative; z-index: 2; }
.pj-hero-eyebrow {
...
.pj-hero-eyebrow span { width: 6px; height: 6px; border-radius: 50%; background: #4ade80; animation: blink 1.8s infinite; }
...
.pj-hero-title span { color: #93c5fd; }
.pj-hero-sub {
  font-family: var(--fb); font-size: .9rem;
  color: rgba(255,255,255,.72); line-height: 1.7; max-width: 540px;
}
.pj-plan-badge {
  ...
  font-family: var(--fb); font-size: .82rem; color: rgba(255,255,255,.9);
}
.pj-plan-badge strong { color: #fde68a; font-family: var(--fh); font-weight: 800; }
...
.pj-step-name {
  ...
  color: var(--ink4); margin-top: 9px; text-align: center;
  white-space: nowrap; letter-spacing: .03em;
  transition: color .3s;
}
.pj-step.active .pj-step-name { color: var(--blue); }
.pj-step.done  .pj-step-name  { color: var(--green); }

/* ══════════════════════════════
   FORM AREA & TABS
══════════════════════════════ */
.pj-form-area { margin-top: 28px; }
.pj-tab { display: none; }
.pj-tab.active {
  display: block;
...
.pj-card:hover { box-shadow: var(--sh-md); }
...
.pj-head-title {
  ...
  color: var(--ink); margin-bottom: 2px; letter-spacing: -.2px;
}
.pj-head-sub {
  font-family: var(--fb); font-size: .78rem; color: var(--ink3);
}
.pj-card-body { padding: 28px 26px; }

/* ══════════════════════════════
   GRID LAYOUTS
══════════════════════════════ */
.pj-row2 { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
.pj-row3 { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 18px; }
.pj-col  { display: flex; flex-direction: column; gap: 0; }

/* ══════════════════════════════
   FIELD GROUP
══════════════════════════════ */
.pj-fg { margin-bottom: 24px; }
.pj-fg:last-child { margin-bottom: 0; }
...
.pj-lbl {
  ...
  color: var(--ink2); margin-bottom: 9px; letter-spacing: -.1px;
}
.pj-lbl-ico {
...
.pj-req { color: var(--red); font-size: .85em; margin-left: 1px; }
.pj-opt {
  font-family: var(--fb); font-size: .66rem; font-weight: 500;
  color: var(--ink4); background: var(--surface); border: 1px solid var(--border);
...
══════════════════════════════ */
.pj-iw { position: relative; }
.pj-iw-prefix {
  position: absolute; left: 0; top: 0; bottom: 0;
  width: 44px;
  display: flex; align-items: center; justify-content: center;
  color: var(--ink4); font-size: .78rem;
  pointer-events: none; z-index: 2;
}
.pj-iw-prefix.top { align-items: flex-start; padding-top: 13px; }
...
.pj-input:hover:not(:focus):not(.err) { border-color: var(--border-h); }
.pj-input:focus {
  border-color: var(--blue);
  box-shadow: 0 0 0 4px var(--blue-ring);
  background: #fafcff;
}
.pj-input::placeholder { color: var(--ink5); font-family: var(--fb); }
.pj-input.no-ico  { padding-left: 14px; }
.pj-input.err     { border-color: var(--red); box-shadow: 0 0 0 4px rgba(220,38,38,.07); }
...
textarea.pj-input {
  padding-top: 13px; resize: vertical; min-height: 130px; line-height: 1.7;
}

/* counter */
.pj-counter {
  font-family: var(--fb); font-size: .7rem; color: var(--ink4);
  text-align: right; margin-top: 6px;
}
.pj-counter.warn { color: var(--amber); }
.pj-counter.over { color: var(--red); }

/* hint */
.pj-hint {
  display: flex; align-items: flex-start; gap: 6px;
  font-family: var(--fb); font-size: .75rem; color: var(--ink3);
  margin-top: 7px; line-height: 1.55;
}
.pj-hint i { color: var(--blue); font-size: .68rem; margin-top: 2px; flex-shrink: 0; }

/* field error */
.pj-ferr {
  display: none; align-items: center; gap: 6px;
  font-family: var(--fb); font-size: .75rem; color: var(--red);
  margin-top: 6px;
}
.pj-ferr.show { display: flex; }

/* alert */
.pj-alert {
  display: none; align-items: flex-start; gap: 12px;
  background: var(--red-lt); border: 1.5px solid var(--red-mid);
  border-radius: var(--r-sm); padding: 14px 18px;
  font-family: var(--fb); font-size: .84rem; color: #b91c1c;
  margin-bottom: 22px;
  animation: shake .38s var(--ease);
}
.pj-alert.show { display: flex; }
@keyframes shake {
...
.pj-sec-divider hr { flex: 1; border: none; height: 1px; background: var(--border); }
.pj-sec-lbl {
  display: flex; align-items: center; gap: 7px;
  font-family: var(--fh); font-size: .68rem; font-weight: 800;
  color: var(--ink3); letter-spacing: .09em; text-transform: uppercase;
...
.pj-type-opt input { display: none; }
.pj-type-opt label {
...
.pj-type-opt label:hover { border-color: var(--blue-mid); transform: translateY(-2px); box-shadow: var(--sh-sm); }
.pj-type-opt label:hover::before { opacity: 1; }
.pj-type-ico {
...
.pj-type-ds {
  font-family: var(--fb); font-size: .72rem; color: var(--ink4);
...
.pj-type-opt input:checked + label .pj-type-nm { color: var(--blue-d); }

/* 2-col type (status) */
.pj-type2 { grid-template-columns: 1fr 1fr; }
...
══════════════════════════════ */
.pj-skills-wrap { position: relative; }
.pj-skills-field {
  border: 1.5px solid var(--border);
  border-radius: var(--r);
  padding: 10px 12px;
  min-height: 56px; cursor: text;
  background: var(--white);
  transition: border-color .2s var(--ease), box-shadow .2s var(--ease), background .2s;
}
...
.pj-chips-row { display: flex; flex-wrap: wrap; gap: 7px; margin-bottom: 7px; }
.pj-chip {
  display: inline-flex; align-items: center; gap: 7px;
  background: linear-gradient(135deg, var(--blue-lt), #e8f0fe);
  border: 1.5px solid var(--blue-mid);
  border-radius: 8px; padding: 5px 10px 5px 13px;
  font-family: var(--fh); font-size: .77rem; font-weight: 700;
  color: var(--blue-d);
...
.pj-chip-rm {
  ...
  color: var(--blue-mid); font-size: .7rem; padding: 0;
  display: flex; align-items: center;
  transition: color .15s; border-radius: 4px;
}
.pj-chip-rm:hover { color: var(--red); }
.pj-skills-input {
  border: none; outline: none; background: transparent;
  font-family: var(--fb); font-size: .9rem; color: var(--ink);
  width: 100%; min-width: 160px;
}
.pj-skills-input::placeholder { color: var(--ink5); }
...
.pj-skills-dd {
  ...
  z-index: 80; max-height: 230px; overflow-y: auto;
  display: none;
}
.pj-skills-dd.open { display: block; animation: ddIn .18s var(--ease); }
...
.pj-dd-item:hover { background: var(--blue-lt); color: var(--blue-d); }
.pj-dd-item i { font-size: .68rem; color: var(--ink5); }
.pj-dd-empty {
  padding: 16px 18px;
...
.pj-sug-label::after { content:''; flex:1; height:1px; background:var(--border); }
.pj-sug-grid { display: flex; flex-wrap: wrap; gap: 8px; }
.pj-sug {
...
.pj-sq-add:hover { background: var(--blue-mid); transform: translateY(-1px); box-shadow: var(--sh-sm); }

.pj-sq-empty {
  border: 2px dashed var(--border);
  border-radius: var(--r-lg); padding: 50px 24px; text-align: center;
  color: var(--ink4);
}
.pj-sq-empty-ico { font-size: 2.2rem; opacity: .25; display: block; margin-bottom: 14px; }
.pj-sq-empty p { font-family: var(--fb); font-size: .86rem; line-height: 1.6; }

.pj-sq-list { display: flex; flex-direction: column; gap: 16px; }
...
.pj-sq-block:hover { box-shadow: var(--sh-md); }
...
.pj-sq-num {
  ...
  font-family: var(--fh); font-size: .72rem; font-weight: 900; flex-shrink: 0;
}
.pj-sq-prev {
  font-family: var(--fh); font-size: .84rem; font-weight: 700;
...
.pj-sq-badge.mc { background: var(--purple-lt); color: var(--purple); }
.pj-sq-badge.tx { background: var(--green-lt); color: var(--green); }
.pj-sq-badge.yn { background: var(--amber-lt); color: var(--amber); }
.pj-sq-del {
  width: 30px; height: 30px; border-radius: 8px;
  border: 1.5px solid var(--border); background: var(--white);
  color: var(--ink4); cursor: pointer; font-size: .72rem;
  display: flex; align-items: center; justify-content: center;
  transition: all .18s;
}
.pj-sq-del:hover { border-color: var(--red-mid); color: var(--red); background: var(--red-lt); }
.pj-sq-body { padding: 20px 18px; }

.pj-sq-type-row {
  display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px;
  margin-bottom: 18px;
}
.pj-sq-topt input { display: none; }
.pj-sq-topt label {
  display: flex; align-items: center; justify-content: center; gap: 7px;
  border: 2px solid var(--border); border-radius: var(--r-sm);
  padding: 10px; font-family: var(--fh); font-size: .78rem; font-weight: 700;
...
.pj-sq-topt label:hover { border-color: var(--blue-mid); }

.pj-sq-opts-lbl {
  font-family: var(--fh); font-size: .66rem; font-weight: 800;
  color: var(--ink4); letter-spacing: .09em; text-transform: uppercase;
  margin-bottom: 10px;
}
.pj-sq-opt-row { display: flex; align-items: center; gap: 8px; margin-bottom: 8px; }
.pj-opt-ltr {
  width: 28px; height: 28px; border-radius: 7px; background: var(--surface);
  display: flex; align-items: center; justify-content: center;
  font-family: var(--fh); font-size: .7rem; font-weight: 800; color: var(--ink3); flex-shrink: 0;
}
.pj-opt-field {
  flex: 1; border: 1.5px solid var(--border); border-radius: var(--r-sm);
  padding: 9px 13px; font-family: var(--fb); font-size: .85rem;
  color: var(--ink); outline: none; transition: border-color .18s, box-shadow .18s;
}
.pj-opt-field:focus { border-color: var(--blue); box-shadow: 0 0 0 3px var(--blue-ring); }
.pj-opt-rm {
  background: none; border: none; color: var(--ink4); cursor: pointer;
  font-size: .72rem; padding: 5px 7px; border-radius: 6px; transition: all .15s;
}
.pj-opt-rm:hover { color: var(--red); background: var(--red-lt); }
.pj-add-opt {
  display: inline-flex; align-items: center; gap: 6px;
  border: 1.5px dashed var(--border); border-radius: var(--r-sm);
  background: none; padding: 8px 14px; margin-top: 6px;
  font-family: var(--fh); font-size: .77rem; font-weight: 700;
  color: var(--ink3); cursor: pointer; transition: all .18s;
}
.pj-add-opt:hover { border-color: var(--blue); color: var(--blue); }

.pj-txt-note {
  background: var(--surface); border: 1.5px dashed var(--border);
  border-radius: var(--r-sm); padding: 13px 16px;
  font-family: var(--fb); font-size: .82rem; color: var(--ink4);
  display: flex; align-items: center; gap: 9px; margin-top: 10px;
}

/* preset screening questions */
.pj-sq-presets { margin-bottom: 20px; }
.pj-preset-lbl {
  font-family: var(--fh); font-size: .7rem; font-weight: 800;
  color: var(--ink3); letter-spacing: .08em; text-transform: uppercase;
  margin-bottom: 10px;
}
.pj-preset-grid { display: flex; flex-wrap: wrap; gap: 8px; }
.pj-preset-btn {
  display: inline-flex; align-items: center; gap: 6px;
  background: var(--white); border: 1.5px solid var(--border);
  border-radius: var(--r-sm); padding: 7px 13px;
  font-family: var(--fh); font-size: .74rem; font-weight: 700;
  color: var(--ink2); cursor: pointer; transition: all .18s var(--ease);
}
.pj-preset-btn i { font-size: .62rem; color: var(--blue); }
.pj-preset-btn:hover { border-color: var(--blue); color: var(--blue); background: var(--blue-lt); transform: translateY(-1px); }
...
.pj-rev-hero::after { content:''; position:absolute; top:-50px; right:-50px; width:200px; height:200px; border-radius:50%; background:rgba(255,255,255,.06); pointer-events:none; }
.pj-rev-title { font-family: var(--fh); font-size: 1.25rem; font-weight: 900; margin-bottom: 4px; position: relative; z-index: 1; }
.pj-rev-co { font-size: .84rem; color: rgba(255,255,255,.72); margin-bottom: 13px; position: relative; z-index: 1; }
.pj-rev-chips { display: flex; flex-wrap: wrap; gap: 8px; position: relative; z-index: 1; }
.pj-rev-chip {
  display: inline-flex; align-items: center; gap: 5px;
...
.pj-rev-box.full { grid-column: 1 / -1; }
.pj-rev-lbl {
  font-family: var(--fh); font-size: .62rem; font-weight: 800;
  color: var(--ink4); text-transform: uppercase; letter-spacing: .08em; margin-bottom: 4px;
}
.pj-rev-val {
  font-family: var(--fh); font-size: .88rem; font-weight: 700; color: var(--ink);
}
...
.pj-terms-box input[type="checkbox"] { width: 18px; height: 18px; margin-top: 1px; accent-color: var(--blue); cursor: pointer; flex-shrink: 0; }
.pj-terms-box label { font-family: var(--fb); font-size: .84rem; color: #065f46; line-height: 1.6; cursor: pointer; }
.pj-terms-box a { color: var(--blue); font-weight: 700; }
...
.pj-footer-info {
  font-family: var(--fb); font-size: .82rem; color: var(--ink3);
  font-weight: 500;
}
.pj-footer-info strong { color: var(--blue); font-family: var(--fh); font-weight: 800; }
.pj-footer-right { display: flex; gap: 10px; align-items: center; flex-wrap: wrap; }

.btn-draft {
  background: none; border: none; cursor: pointer;
  font-family: var(--fb); font-size: .8rem; font-weight: 600;
  color: var(--ink3); padding: 9px 14px; border-radius: var(--r-sm);
...
.btn-next:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(37,99,235,.35); }
.btn-publish {
  background: linear-gradient(135deg, #0f2460, var(--blue-d), var(--blue));
  color: var(--white); border: none; border-radius: var(--r-sm);
  padding: 12px 30px;
  font-family: var(--fh); font-size: .92rem; font-weight: 900;
  cursor: pointer; display: flex; align-items: center; gap: 9px;
  transition: all .25s var(--ease);
  box-shadow: 0 4px 16px rgba(37,99,235,.3);
  letter-spacing: -.2px;
}
.btn-publish:hover { transform: translateY(-2px); box-shadow: 0 8px 28px rgba(37,99,235,.4); }
.btn-publish:disabled { opacity: .6; cursor: not-allowed; transform: none; box-shadow: none; }

/* ══════════════════════════════
   RESPONSIVE
══════════════════════════════ */
@media(max-width:720px){
  .pj-hero { padding: 24px 22px 88px; }
  .pj-wizard-card { padding: 16px 14px; }
  .pj-step-name { font-size: .52rem; }
  .pj-card-body { padding: 18px 16px; }
  .pj-card-head { padding: 14px 16px; }
  .pj-row2, .pj-row3, .pj-type-grid, .pj-sq-type-row { grid-template-columns: 1fr; }
  .pj-type2 { grid-template-columns: 1fr 1fr; }
  .pj-salary-row { grid-template-columns: 1fr 1fr; }
  .pj-salary-sep, .pj-salary-tag { display: none; }
  .pj-rev-grid { grid-template-columns: 1fr; }
  .pj-footer { flex-direction: column; }
  .pj-footer-right { width: 100%; justify-content: flex-end; }
  .btn-next, .btn-back, .btn-publish { width: 100%; justify-content: center; }
}
</style>
@endpush
  ...
  <div class="pj-hero">
    ...
    <div class="pj-hero-inner">
      ...
      <div class="pj-hero-sub">
        Reach thousands of skilled professionals across Tamil Nadu. Complete the 5 steps below and publish your listing in minutes.
      </div>
      @if(isset($activePlan))
      ...
      @else
        <div class="pj-plan-badge" style="background:rgba(239,68,68,.2);border-color:rgba(239,68,68,.35);">
          ...
          <span style="color:#fca5a5;">No active plan — <a href="{{ route('employer.billing') }}" style="color:#fde68a;font-weight:800;">Purchase a plan</a> to post jobs.</span>
        </div>
      @endif
    </div>
  </div>
    ...
    <form method="POST" action="{{ route('employer.jobs.store') }}" id="pjForm" novalidate>
      ...
      <div class="pj-tab active" id="pjTab1">
          ...
          <div class="pj-card-body">
            ...
            <div class="pj-fg">
              ...
              <div class="pj-ferr @error('job_title') show @enderror" id="err-job_title">
                ...
                <span>@error('job_title'){{ $message }}@else Job title is required (min. 3 characters).@enderror</span>
              </div>
              <div class="pj-hint"><i class="fa-solid fa-lightbulb"></i> Be specific — "Senior PHP Developer" attracts better matches than just "Developer"</div>
            </div>
            ...
            <div class="pj-row2">
              <div class="pj-fg">
                ...
                <div class="pj-iw">
                  ...
                  <select id="job_category" name="job_category" class="pj-input @error('job_category') err @enderror">
                    <option value="" disabled {{ old('job_category') ? '' : 'selected' }}>Select Category</option>
                    @foreach(['IT & Software','Technical & Trade','Sales & Marketing','Office & Admin','Driver & Logistics','Manufacturing','Healthcare','Education','Hospitality','Other'] as $cat)
                      <option value="{{ $cat }}" {{ old('job_category')===$cat ? 'selected' : '' }}>{{ $cat }}</option>
                    @endforeach
                  </select>
                </div>
              ...
              <div class="pj-fg">
                ...
                <div class="pj-iw">
                  ...
                  <select id="industry_type" name="industry_type" class="pj-input @error('industry_type') err @enderror">
                    <option value="" disabled {{ old('industry_type') ? '' : 'selected' }}>Select Industry</option>
                ...
                <div class="pj-ferr @error('industry_type') show @enderror" id="err-industry_type">
                  <i class="fa-solid fa-circle-exclamation"></i><span>Please select an industry.</span>
                </div>
              </div>
            </div>
            ...
            <div class="pj-fg">
              ...
              <div class="pj-iw">
                ...
                <textarea id="description" name="description"
                  class="pj-input @error('description') err @enderror"
              ...
              <div style="display:flex;justify-content:space-between;margin-top:6px;align-items:center;">
                <div class="pj-ferr @error('description') show @enderror" id="err-description">
                  <i class="fa-solid fa-circle-exclamation"></i><span>Minimum 50 characters required.</span>
                </div>
            ...
            <div class="pj-fg">
              ...
              <div class="pj-iw">
                ...
                <textarea id="responsibilities" name="responsibilities"
                  class="pj-input @error('responsibilities') err @enderror"
              ...
              <div style="display:flex;justify-content:space-between;margin-top:6px;align-items:center;">
                <div class="pj-ferr @error('responsibilities') show @enderror" id="err-responsibilities">
                  <i class="fa-solid fa-circle-exclamation"></i><span>Please enter responsibilities (min. 20 characters).</span>
                </div>
            ...
            <div class="pj-fg">
              ...
              <div class="pj-iw">
                ...
                <textarea id="benefits" name="benefits" class="pj-input"
                  style="padding-left:44px;min-height:90px;" maxlength="500"
        ...
        <div class="pj-footer">
          ...
          <div class="pj-footer-right">
            <button type="button" class="btn-draft" onclick="saveDraft(this)"><i class="fa-regular fa-floppy-disk"></i> Save Draft</button>
            <button type="button" class="btn-next" onclick="nextStep(1)">Location &nbsp;<i class="fa-solid fa-arrow-right"></i></button>
          </div>
        </div>
      </div>
      ...
      <div class="pj-tab" id="pjTab2">
            ...
            <div class="pj-row3">
              <div class="pj-fg">
                ...
                <div class="pj-iw">
                  ...
                  <select id="state" name="state" class="pj-input @error('state') err @enderror">
                    ...
                    @foreach(['Tamil Nadu','Kerala','Karnataka','Andhra Pradesh','Telangana'] as $st)
                      <option value="{{ $st }}" {{ old('state','Tamil Nadu')===$st ? 'selected' : '' }}>{{ $st }}</option>
                    @endforeach
                  </select>
                </div>
              ...
              <div class="pj-fg">
                ...
                <div class="pj-iw">
                  ...
                  <select id="district" name="district" class="pj-input @error('district') err @enderror">
                ...
                <div class="pj-ferr @error('district') show @enderror" id="err-district">
                  <i class="fa-solid fa-circle-exclamation"></i><span>Please select a district.</span>
                </div>
              </div>
              <div class="pj-fg">
                ...
                <div class="pj-iw">
                  ...
                  <input type="text" id="city" name="city"
                    class="pj-input @error('city') err @enderror"
                    placeholder="e.g. Coimbatore"
                    value="{{ old('city') }}" />
                </div>
                <div class="pj-ferr @error('city') show @enderror" id="err-city">
                  <i class="fa-solid fa-circle-exclamation"></i><span>Please enter the city / town name.</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="pj-footer">
          ...
          <div class="pj-footer-right">
            <button type="button" class="btn-draft" onclick="saveDraft(this)"><i class="fa-regular fa-floppy-disk"></i> Save Draft</button>
            <button type="button" class="btn-back" onclick="prevStep(2)"><i class="fa-solid fa-arrow-left"></i> Back</button>
            <button type="button" class="btn-next" onclick="nextStep(2)">Job Info &amp; Skills &nbsp;<i class="fa-solid fa-arrow-right"></i></button>
          </div>
        </div>
      </div>
      ...
      <div class="pj-tab" id="pjTab3">
        ...
        <div class="pj-card">
          <div class="pj-card-head">
            ...
            <div class="pj-head-text">
              ...
              <div class="pj-head-sub">Experience, salary, vacancies, type, education and required skills</div>
            </div>
          </div>
          <div class="pj-card-body">
            ...
            <div class="pj-row2">
              <div class="pj-fg">
                ...
                <div class="pj-iw">
                  <div class="pj-iw-prefix"><i class="fa-solid fa-briefcase"></i></div>
                  <select id="experience_required" name="experience_required" class="pj-input @error('experience_required') err @enderror">
                    <option value="" disabled {{ old('experience_required') ? '' : 'selected' }}>Select Experience</option>
                    @foreach(['Fresher','Less than 1 year','1-2 Years','2-3 Years','3-5 Years','5-8 Years','8-10 Years','10+ Years'] as $ex)
                      <option value="{{ $ex }}" {{ old('experience_required')===$ex ? 'selected' : '' }}>{{ $ex }}</option>
                    @endforeach
                  </select>
                </div>
                <div class="pj-ferr @error('experience_required') show @enderror" id="err-experience_required">
                  <i class="fa-solid fa-circle-exclamation"></i><span>Please select experience level.</span>
                </div>
              </div>
              <div class="pj-fg">
                ...
                <div class="pj-iw">
                  ...
                  <input type="number" id="vacancies" name="vacancies"
                    class="pj-input @error('vacancies') err @enderror"
                    placeholder="e.g. 3" min="1" max="100"
                    value="{{ old('vacancies', 1) }}" />
                </div>
                <div class="pj-ferr @error('vacancies') show @enderror" id="err-vacancies">
                  <i class="fa-solid fa-circle-exclamation"></i><span>At least 1 vacancy required.</span>
                </div>
              </div>
            </div>
            ...
            <div class="pj-fg">
              ...
              <div class="pj-iw">
                ...
                <select id="salary_range" name="salary_range" class="pj-input @error('salary_range') err @enderror">
                  <option value="" disabled {{ old('salary_range') ? '' : 'selected' }}>Select Salary Range</option>
                  @php
                    $salaryRanges = [
                  ...
                  @foreach($salaryRanges as $lbl => $val)
                    <option value="{{ $val }}" {{ old('salary_range')===$val ? 'selected' : '' }}>{{ $lbl }}</option>
                  @endforeach
                </select>
              </div>
              <div class="pj-ferr @error('salary_range') show @enderror" id="err-salary_range">
                <i class="fa-solid fa-circle-exclamation"></i><span>Please select a salary range.</span>
              </div>
              <div class="pj-hint"><i class="fa-solid fa-circle-info"></i> Select "Not Disclosed" if you prefer not to show salary publicly</div>
            </div>
            ...
            <div class="pj-fg">
              ...
              <div class="pj-iw">
                ...
                <select id="education" name="education" class="pj-input @error('education') err @enderror">
                  <option value="" disabled {{ old('education') ? '' : 'selected' }}>Select Education Requirement</option>
                  @foreach(['None (No Requirement)'=>'None','10th Pass (SSLC)'=>'10th','12th Pass (HSC / +2)'=>'12th','Diploma'=>'Diploma',"Bachelor's Degree (UG)"=>'Bachelor',"Master's Degree (PG)"=>'Master','Doctorate / PhD'=>'Doctorate'] as $lbl=>$val)
                    <option value="{{ $val }}" {{ old('education')===$val ? 'selected' : '' }}>{{ $lbl }}</option>
                  @endforeach
                </select>
              </div>
              <div class="pj-ferr @error('education') show @enderror" id="err-education">
                <i class="fa-solid fa-circle-exclamation"></i><span>Please select education requirement.</span>
              </div>
            </div>
            ...
            <div class="pj-fg">
              ...
              <div class="pj-type-grid">
                @foreach([
                  ['Full Time','fa-clock','Standard working hours, Monday to Saturday'],
                  ['Part Time','fa-hourglass-half','Flexible or reduced hours schedule'],
                  ['Contract','fa-file-contract','Fixed-term or project-based engagement'],
                ] as [$jt,$ico,$desc])
                  <div class="pj-type-opt">
                    <input type="radio" id="jt_{{ Str::slug($jt) }}" name="job_type" value="{{ $jt }}" {{ old('job_type','Full Time')===$jt ? 'checked' : '' }}>
                    <label for="jt_{{ Str::slug($jt) }}">
                      <div class="pj-type-ico"><i class="fa-solid {{ $ico }}"></i></div>
            ...
            <div class="pj-fg">
              ...
              <div class="pj-type-grid pj-type2" style="max-width:460px;">
                @foreach([
                  ...
                  <div class="pj-type-opt">
                    <input type="radio" id="status_{{ $val }}" name="status" value="{{ $val }}" {{ old('status','active')===$val ? 'checked' : '' }}>
            ...
            <div class="pj-fg">
              ...
              <div class="pj-skills-wrap">
                <div class="pj-skills-field" id="skillsField" onclick="document.getElementById('skillInput').focus()">
              ...
              <div class="pj-ferr" id="err-skills"><i class="fa-solid fa-circle-exclamation"></i><span>Please add at least one required skill.</span></div>
              <div class="pj-hint" style="margin-top:8px;"><i class="fa-solid fa-circle-info"></i> Add up to 10 skills. Press Enter or comma to add a custom skill.</div>
            </div>

            <div style="margin-top:18px;">
              <div class="pj-sug-label"><i class="fa-solid fa-fire" style="color:var(--amber);"></i> Popular Suggestions</div>
        ...
        <div class="pj-footer">
          ...
          <div class="pj-footer-right">
            <button type="button" class="btn-draft" onclick="saveDraft(this)"><i class="fa-regular fa-floppy-disk"></i> Save Draft</button>
            <button type="button" class="btn-back" onclick="prevStep(3)"><i class="fa-solid fa-arrow-left"></i> Back</button>
            <button type="button" class="btn-next" onclick="nextStep(3)">Screening &nbsp;<i class="fa-solid fa-arrow-right"></i></button>
          </div>
        </div>
      </div>
        ...
        <div class="pj-card">
          <div class="pj-card-head">
            ...
            <div class="pj-head-text">
              ...
              <div class="pj-head-sub">Candidates must answer these when applying — optional but highly effective</div>
            </div>
          </div>
          <div class="pj-card-body">
            ...
            <div class="pj-sq-presets">
              <div class="pj-preset-lbl"><i class="fa-solid fa-wand-magic-sparkles" style="color:var(--blue);"></i> Quick-add preset questions</div>
              <div class="pj-preset-grid" id="presetGrid">
                @php
                  $presets = [
                    ['Do you have a valid driving licence?','select',['Yes','No']],
                    ['Are you comfortable with night / rotational shifts?','select',['Yes','No','Depends on the schedule']],
                    ['What is your current salary (CTC per month)?','input',[]],
                    ['How many years of relevant experience do you have?','input',[]],
                    ['Are you willing to relocate for this position?','select',['Yes, open to relocation','No, prefer local','Need more details']],
                    ['Do you have a two-wheeler / vehicle?','select',['Yes, own vehicle','No']],
                    ['When can you join?','select',['Immediately','Within 15 days','Within 30 days','More than 30 days']],
                    ['Do you have experience with Tally / ERP software?','select',['Yes','No','Currently learning']],
                    ['What is your highest qualification?','select',['10th Pass','12th Pass','Diploma','Degree','Post-Graduate']],
                    ['Why are you interested in this role?','input',[]],
                  ];
                @endphp
                @foreach($presets as $pi => [$pq,$pt,$po])
                  <button type="button" class="pj-preset-btn" onclick='addPresetSQ({{ $pi }})'>
            ...
            <div class="pj-sq-empty" id="sqEmpty">
              ...
              <p>No screening questions yet.<br>Use a preset above or click <strong>Add Custom</strong> to create your own.</p>
            </div>

            <input type="hidden" name="screening_questions" id="sqData" value="{{ old('screening_questions','[]') }}" />
          </div>
        </div>
        <div class="pj-footer">
          ...
          <div class="pj-footer-right">
            <button type="button" class="btn-draft" onclick="saveDraft(this)"><i class="fa-regular fa-floppy-disk"></i> Save Draft</button>
            <button type="button" class="btn-back" onclick="prevStep(4)"><i class="fa-solid fa-arrow-left"></i> Back</button>
            <button type="button" class="btn-next" onclick="nextStep(4)">Review &amp; Publish &nbsp;<i class="fa-solid fa-arrow-right"></i></button>
          </div>
        </div>
      </div>
        ...
        <div class="pj-card">
          ...
          <div class="pj-card-body">
            <div class="pj-rev-hero">
              ...
              <div class="pj-rev-co">{{ auth()->user()->company->name ?? (auth()->user()->name ?? 'Your Company') }} &middot; <span id="rev-location">—</span></div>
            ...
            <div class="pj-rev-grid">
              @foreach([
                ...
                ['rev-industry','Industry'],
                ['rev-exp','Experience'],
                ['rev-salary','Salary Range'],
                ['rev-type','Job Type'],
                ['rev-education','Education'],
                ['rev-vacancies','Vacancies'],
                ['rev-status','Status'],
              ] as [$rid,$rlbl])
            ...
            <div class="pj-ferr" id="err-terms" style="margin-top:8px;">
              <i class="fa-solid fa-circle-exclamation"></i><span>Please accept the terms and conditions to publish.</span>
            </div>
          </div>
        </div>
        <div class="pj-footer">
          ...
          <div class="pj-footer-right">
            <button type="button" class="btn-back" onclick="prevStep(5)"><i class="fa-solid fa-arrow-left"></i> Back</button>
...
window.nextStep = function(from){ if(validateStep(from)) showStep(from+1); };
...
function showFE(id, msg){
  ...
  if(er){ er.classList.add('show'); var sp=er.querySelector('span'); if(sp&&msg) sp.textContent=msg; }
}
function clearTab(n){
  var tab = document.getElementById('pjTab'+n); if(!tab) return;
  ...
  var al = document.getElementById('alert'+n); if(al) al.classList.remove('show');
}
...
function validateStep(n){
  ...
  if(n===1){
    if(gv('job_title').length < 3){ showFE('job_title','Job title must be at least 3 characters.'); ok=false; }
    if(!gv('job_category')){ showFE('job_category','Please select a category.'); ok=false; }
    if(!gv('industry_type')){ showFE('industry_type','Please select an industry.'); ok=false; }
    if((document.getElementById('description')?.value.trim()||'').length < 50){ showFE('description','Minimum 50 characters required.'); ok=false; }
    if((document.getElementById('responsibilities')?.value.trim()||'').length < 20){ showFE('responsibilities','Minimum 20 characters required.'); ok=false; }
  }
  if(n===2){
    if(!gv('state')){ showFE('state','Please select a state.'); ok=false; }
    if(!gv('district')){ showFE('district','Please select a district.'); ok=false; }
    if(gv('city').length < 2){ showFE('city','Please enter the city / town name.'); ok=false; }
  }
  if(n===3){
    if(!gv('experience_required')){ showFE('experience_required','Please select experience level.'); ok=false; }
    var vac = document.getElementById('vacancies')?.value.trim()||'';
    if(!vac || parseInt(vac)<1){ showFE('vacancies','At least 1 vacancy required.'); ok=false; }
    if(!gv('salary_range')){ showFE('salary_range','Please select a salary range.'); ok=false; }
    if(!gv('education')){ showFE('education','Please select education requirement.'); ok=false; }
    document.getElementById('skillsHidden').value = JSON.stringify(selectedSkills);
    if(selectedSkills.length === 0){ document.getElementById('err-skills').classList.add('show'); ok=false; }
  }
  if(n===4){ saveSQData(); }
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  initSkills();
  /* init char counters */
  [['description','descCnt',3000],['responsibilities','respCnt',2000],['benefits','benCnt',500]].forEach(function([id,cid,max]){
    var el = document.getElementById(id); if(el) charCount(el, cid, max);
  });
  /* restore old SQ data */
  var oldSQ = document.getElementById('sqData').value;
  try{ var arr=JSON.parse(oldSQ); arr.forEach(function(q){ addSQFromData(q); }); }catch(e){}
});
...
function initSkills(){
  ...
  if(old){ try{ JSON.parse(old).forEach(s=>{ if(s && selectedSkills.indexOf(s)===-1){ selectedSkills.push(s); renderChip(s); }}); }catch(e){} }
  filterSkills('');
}
...
window.closeSkillsDD = function(){ document.getElementById('skillsDD').classList.remove('open'); };

window.filterSkills = function(q){
  var dd = document.getElementById('skillsDD'); if(!dd) return;
  var flt = ALL_SKILLS.filter(s => s.toLowerCase().includes((q||'').toLowerCase()) && selectedSkills.indexOf(s)===-1);
  if(flt.length === 0){
    dd.innerHTML = q
      ? '<div class="pj-dd-empty"><i class="fa-solid fa-keyboard" style="color:var(--blue);"></i> Press Enter to add "<strong>'+escHtml(q)+'</strong>"</div>'
    ...
    dd.innerHTML = flt.slice(0,10).map(s => {
      ...
      return '<div class="pj-dd-item" onmousedown="event.preventDefault();addSkill(\''+safe+'\')"><i class="fa-solid fa-screwdriver-wrench"></i> '+escHtml(s)+'</div>';
    }).join('');
  }
...
function renderChip(skill){
  ...
  chip.innerHTML = escHtml(skill) +
    '<button type="button" class="pj-chip-rm" onclick="removeSkill(\''+safe+'\')" title="Remove"><i class="fa-solid fa-xmark"></i></button>';
  document.getElementById('skillChips').appendChild(chip);
}

window.removeSkill = function(skill){
  selectedSkills = selectedSkills.filter(s => s!==skill);
  document.querySelectorAll('#skillChips .pj-chip').forEach(c => { if(c.getAttribute('data-skill')===skill) c.remove(); });
  filterSkills(document.getElementById('skillInput').value);
};
...
var PRESETS = [
  {question:'Do you have a valid driving licence?',type:'select',options:['Yes','No']},
  {question:'Are you comfortable with night / rotational shifts?',type:'select',options:['Yes','No','Depends on the schedule']},
  {question:'What is your current salary (CTC per month)?',type:'input',options:[]},
  {question:'How many years of relevant experience do you have?',type:'input',options:[]},
  {question:'Are you willing to relocate for this position?',type:'select',options:['Yes, open to relocation','No, prefer local','Need more details']},
  {question:'Do you have a two-wheeler / vehicle?',type:'select',options:['Yes, own vehicle','No']},
  {question:'When can you join?',type:'select',options:['Immediately','Within 15 days','Within 30 days','More than 30 days']},
  {question:'Do you have experience with Tally / ERP software?',type:'select',options:['Yes','No','Currently learning']},
  {question:'What is your highest qualification?',type:'select',options:['10th Pass','12th Pass','Diploma','Degree','Post-Graduate']},
  {question:'Why are you interested in this role?',type:'input',options:[]},
];

window.addPresetSQ = function(pi){
  var p = PRESETS[pi]; if(!p) return;
  addSQFromData(p);
};
...
function addSQFromData(data){
  ...
  el.className = 'pj-sq-block'; el.id = id;
...
window.addSQ = function(){
  addSQFromData({question:'', type:'select', options:['','']});
};
...
function buildSQHTML(id, num, data){
  ...
  return '<div class="pj-sq-block-top">'+
    ...
    '<span class="pj-sq-badge '+(type==='select'?'mc':type==='yn'?'yn':'tx')+'" id="'+id+'_badge">'+
      (type==='select'?'Multiple Choice':type==='yn'?'Yes / No':'Text Input')+'</span>'+
    '<button type="button" class="pj-sq-del" onclick="delSQ(\''+id+'\')" title="Remove"><i class="fa-solid fa-trash"></i></button>'+
    '</div>'+
    '<div class="pj-sq-body">'+
      ...
      '<input type="text" id="'+id+'_q" class="pj-input" style="padding-left:44px;" placeholder="e.g. Do you have a valid driving licence?" value="'+escAttr(q)+'" '+
      'oninput="(function(el){var p=document.getElementById(\''+id+'_prev\');if(p)p.textContent=el.value||\'Question\';saveSQData();})(this)"/>'+
      '</div></div>'+
      ...
      '<div class="pj-sq-type-row">'+
        '<div class="pj-sq-topt"><input type="radio" id="'+id+'_ts" name="'+id+'_type" value="select" '+(type==='select'?'checked':'')+' onchange="switchSQType(\''+id+'\',\'select\',true)"><label for="'+id+'_ts"><i class="fa-solid fa-list-ul"></i> Multiple Choice</label></div>'+
        '<div class="pj-sq-topt"><input type="radio" id="'+id+'_ti" name="'+id+'_type" value="input" '+(type==='input'?'checked':'')+' onchange="switchSQType(\''+id+'\',\'input\',true)"><label for="'+id+'_ti"><i class="fa-solid fa-keyboard"></i> Text Input</label></div>'+
        '<div class="pj-sq-topt"><input type="radio" id="'+id+'_ty" name="'+id+'_type" value="yn" '+(type==='yn'?'checked':'')+' onchange="switchSQType(\''+id+'\',\'yn\',true)"><label for="'+id+'_ty"><i class="fa-solid fa-circle-half-stroke"></i> Yes / No</label></div>'+
      '</div>'+
      '<div id="'+id+'_selfields">'+
        '<div class="pj-sq-opts-lbl">Answer Options</div>'+
        '<div id="'+id+'_opts">'+optsHTML+'</div>'+
        '<button type="button" class="pj-add-opt" onclick="addOpt(\''+id+'\')" id="'+id+'_addOptBtn"><i class="fa-solid fa-plus"></i> Add Option</button>'+
      '</div>'+
...
function buildOptRow(id, num, ltr, val){
  var rm = num > 2
    ? '<button type="button" class="pj-opt-rm" onclick="removeOpt(\''+id+'\','+num+')" title="Remove option"><i class="fa-solid fa-xmark"></i></button>'
    : '<span style="width:28px;display:inline-block;"></span>';
  return '<div class="pj-sq-opt-row" id="'+id+'_orow_'+num+'">'+
    '<div class="pj-opt-ltr">'+escHtml(ltr)+'</div>'+
    '<input type="text" class="pj-opt-field" id="'+id+'_opt_'+num+'" placeholder="Option '+escHtml(ltr)+'" value="'+escAttr(val)+'" oninput="saveSQData()"/>'+
    rm+'</div>';
}
...
window.addOpt = function(id){
  ...
  var c = document.getElementById(id+'_opts'); if(!c) return;
...
window.removeOpt = function(id, num){
  var r = document.getElementById(id+'_orow_'+num); if(r) r.remove(); saveSQData();
};
...
window.switchSQType = function(id, type, save){
  ...
  if(b){
    if(type==='select'){ b.className='pj-sq-badge mc'; b.textContent='Multiple Choice'; }
    else if(type==='yn'){ b.className='pj-sq-badge yn'; b.textContent='Yes / No'; }
    else { b.className='pj-sq-badge tx'; b.textContent='Text Input'; }
  }
  if(save) saveSQData();
};

window.delSQ = function(id){
  var bl = document.getElementById(id); if(!bl) return;
  bl.style.opacity='0'; bl.style.transform='translateX(-10px)';
...
function renumSQ(){
  document.querySelectorAll('.pj-sq-block').forEach(function(b,i){
    var n = b.querySelector('.pj-sq-num'); if(n) n.textContent='Q'+(i+1);
  });
}
...
function saveSQData(){
  ...
  document.querySelectorAll('.pj-sq-block').forEach(function(bl){
    ...
    var obj = {question:q, type:type, options:[]};
    if(type==='select'){
      bl.querySelectorAll('.pj-opt-field').forEach(function(o){ if(o.value.trim()) obj.options.push(o.value.trim()); });
    } else if(type==='yn'){
      obj.options = ['Yes','No'];
    }
    data.push(obj);
  });
  var h = document.getElementById('sqData'); if(h) h.value=JSON.stringify(data);
}
...
function buildReview(){
  ...
  }catch(e){ set('rev-sq','None added'); }

  var chips = document.getElementById('rev-chips'); if(!chips) return;
  chips.innerHTML='';
  function chip(icon,txt){ return '<span class="pj-rev-chip"><i class="fa-solid '+icon+'"></i> '+escHtml(txt)+'</span>'; }
  if(jtr) chips.innerHTML += chip('fa-clock', jtr.value);
  if(gv('salary_range') && gv('salary_range')!=='Not Disclosed') chips.innerHTML += chip('fa-indian-rupee-sign', gv('salary_range'));
  if(gv('experience_required')) chips.innerHTML += chip('fa-briefcase', gv('experience_required'));
  if(gv('city')) chips.innerHTML += chip('fa-location-dot', [gv('city'),gv('state')].filter(Boolean).join(', '));
}
...
document.getElementById('pjForm').addEventListener('submit', function(e){
  ...
  if(!tc || !tc.checked){ e.preventDefault(); if(et) et.classList.add('show'); return; }
...
window.saveDraft = function(btn){
  ...
  setTimeout(function(){
    ...
    setTimeout(function(){ btn.innerHTML='<i class="fa-regular fa-floppy-disk"></i> Save Draft'; btn.disabled=false; },2000);
  },900);
};
