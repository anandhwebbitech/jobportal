...
.lj-auth-left{background:linear-gradient(155deg,#059669 0%,#065f46 100%);display:flex;flex-direction:column;justify-content:center;padding:60px 48px;position:relative;overflow:hidden;}
...
.lj-auth-logo{font-size:1.45rem;font-weight:800;color:var(--green);letter-spacing:-1px;margin-bottom:4px;display:flex;align-items:center;gap:9px;}
.lj-auth-logo-ico{width:36px;height:36px;border-radius:var(--r-sm);background:var(--green-lt);color:var(--green);display:flex;align-items:center;justify-content:center;font-size:.88rem;}
...
.lj-input:focus{border-color:var(--green);box-shadow:0 0 0 3px rgba(5,150,105,.1);}
...
.lj-check input{width:15px;height:15px;accent-color:var(--green);}
.lj-forgot{font-size:.85rem;font-weight:600;color:var(--green);text-decoration:none;}
.lj-forgot:hover{text-decoration:underline;}
.lj-submit{width:100%;background:var(--green);color:#fff;border:none;border-radius:var(--r);font-family:var(--f);font-size:.9375rem;font-weight:700;padding:13px 20px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:10px;transition:background var(--t),transform var(--t),box-shadow var(--t);margin-bottom:20px;}
.lj-submit:hover{background:var(--green-h);transform:translateY(-1px);box-shadow:0 4px 16px rgba(5,150,105,.28);}
.lj-auth-switch{text-align:center;font-size:.875rem;color:var(--n500);}
.lj-auth-switch a{color:var(--green);font-weight:600;text-decoration:none;}
...
@media(max-width:900px){.lj-auth-split{grid-template-columns:1fr;}.lj-auth-left{display:none;}.lj-auth-right{padding:40px 24px;}}
</style>
@endpush
...
<div class="lj-auth-page">
  <div class="lj-auth-split">
    ...
    <div class="lj-auth-left">
      ...
      <div class="lj-al-badge"><i class="fa-solid fa-building"></i> Employer Portal</div>
      <div class="lj-al-title">Find the Right Talent for Your Business</div>
      <div class="lj-al-sub">Post jobs and connect with thousands of skilled professionals across Tamil Nadu — starting at just ₹600.</div>
      <div class="lj-al-perks">
        <div class="lj-al-perk">
          <div class="lj-al-perk-ico"><i class="fa-solid fa-users"></i></div>
          <div class="lj-al-perk-text"><strong>Large Talent Pool</strong>Access 50,000+ registered job seekers across all skills.</div>
        </div>
        <div class="lj-al-perk">
          <div class="lj-al-perk-ico"><i class="fa-solid fa-tag"></i></div>
          <div class="lj-al-perk-text"><strong>Affordable Plans</strong>Post jobs from ₹600. Plans designed for MSMEs.</div>
        </div>
        <div class="lj-al-perk">
          <div class="lj-al-perk-ico"><i class="fa-solid fa-map-location-dot"></i></div>
          <div class="lj-al-perk-text"><strong>Local Hiring Focus</strong>Reach candidates in all 32 districts of Tamil Nadu.</div>
        </div>
        <div class="lj-al-perk">
          <div class="lj-al-perk-ico"><i class="fa-solid fa-chart-line"></i></div>
          <div class="lj-al-perk-text"><strong>Easy Dashboard</strong>Manage all applications from one simple dashboard.</div>
        </div>
      </div>
      <div class="lj-al-bottom">
        <div><div class="lj-al-stat-val">1,200+</div><div class="lj-al-stat-lbl">Employers</div></div>
        <div><div class="lj-al-stat-val">₹600</div><div class="lj-al-stat-lbl">Starting Plan</div></div>
        <div><div class="lj-al-stat-val">48hr</div><div class="lj-al-stat-lbl">Avg. Response</div></div>
      </div>
    </div>
        ...
        <form method="POST" action="{{ route('employer.login.submit') }}">
          ...
          <div class="lj-fgroup">
            ...
            <div class="lj-iw">
              ...
              <input type="email" id="email" name="email" class="lj-input @error('email') is-invalid @enderror" placeholder="company@example.com" value="{{ old('email') }}" required autocomplete="email" />
            </div>
          </div>
          <div class="lj-fgroup">
            ...
            <div class="lj-iw">
              ...
              <input type="password" id="password" name="password" class="lj-input pr @error('password') is-invalid @enderror" placeholder="Enter your password" required autocomplete="current-password" />
          ...
          <div class="lj-row-check">
            <label class="lj-check"><input type="checkbox" name="remember"> Remember me</label>
            <a href="{{ route('forgot.password') }}" class="lj-forgot">Forgot password?</a>
          </div>
          <button type="submit" class="lj-submit"><i class="fa-solid fa-right-to-bracket"></i> Login to Dashboard</button>
        </form>

        <div class="lj-or">or</div>
        <div class="lj-auth-switch">New employer? <a href="{{ route('employer.register') }}">Register Your Company</a></div>
        <div class="lj-auth-switch" style="margin-top:10px;">Looking for a job? <a href="{{ route('jobseeker.login') }}">Job Seeker Login</a></div>
      </div>
    </div>
...
@push('scripts')
<script>
function togglePwd(id,btn){const inp=document.getElementById(id);const ico=btn.querySelector('i');if(inp.type==='password'){inp.type='text';ico.className='fa-solid fa-eye-slash';}else{inp.type='password';ico.className='fa-solid fa-eye';}}
</script>
@endpush