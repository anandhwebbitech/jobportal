{{-- ═══════════════════════════════════════════════════════
     resources/views/frontend/jobseeker/alerts.blade.php
     Job Alerts – LinearJobs Job Seeker Dashboard
═══════════════════════════════════════════════════════ --}}
...
<div class="lj-grid-2">
  ...
  <div class="lj-card lj-section-gap">
    <div class="lj-card-head">
      ...
      <span style="font-size:.75rem;color:var(--n500);">{{ ($alerts ?? collect())->count() }} alert(s)</span>
    </div>
      ...
      @forelse($alerts ?? [] as $alert)
        <div class="lj-alert-item">
          ...
          <div style="display:flex;align-items:center;gap:10px;flex-shrink:0;">
            ...
            <form method="POST" action="{{ route('jobseeker.dashboard.alerts.delete', $alert->id) }}"
  ...
  <div class="lj-card">
    ...
    <form method="POST" action="{{ route('jobseeker.dashboard.alerts.store') }}">
      @csrf
      <div class="lj-card-body">
        <div class="lj-fgroup" style="margin-bottom:14px;">
          ...
          <div class="lj-input-wrap">
            ...
            <input class="lj-input has-ico" type="text" name="keyword"
              value="{{ old('keyword') }}"
              placeholder="e.g. PHP Developer, Laravel" required/>
          </div>
          @error('keyword')<div style="font-size:.73rem;color:var(--red);margin-top:4px;">{{ $message }}</div>@enderror
        </div>
        ...
        <div style="margin-top:14px;padding:12px 14px;background:var(--blue-light);border:1.5px solid var(--blue-mid);border-radius:8px;font-size:.78rem;color:#1e40af;display:flex;align-items:center;gap:8px;">
          <i class="fa-solid fa-envelope"></i>
          Alerts will be sent to <strong>{{ auth()->user()->email }}</strong>
        </div>
      </div>
...
{{-- Alert matches info --}}
@if(($totalMatches ?? 0) > 0)
  <div style="background:var(--green-light);border:1.5px solid #86efac;border-radius:12px;padding:16px 20px;display:flex;align-items:center;gap:14px;">
    <div style="width:44px;height:44px;border-radius:50%;background:#dcfce7;display:flex;align-items:center;justify-content:center;font-size:1rem;color:var(--green);flex-shrink:0;">
      <i class="fa-solid fa-circle-check"></i>
    </div>
    <div>
      <div style="font-weight:700;color:#166534;">{{ $totalMatches }} new jobs match your alerts!</div>
      <div style="font-size:.78rem;color:#16a34a;margin-top:2px;">
