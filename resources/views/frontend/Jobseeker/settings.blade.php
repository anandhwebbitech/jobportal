...
<div class="lj-grid-2">
    ...
    <form method="POST" action="{{ route('jobseeker.settings.password') }}" id="passwordForm">
      ...
      <div class="lj-card-body">
        ...
        <div class="lj-fgroup" style="margin-bottom:14px;">
          ...
          @error('current_password')<div style="font-size:.72rem;color:var(--red);margin-top:3px;">{{ $message }}</div>@enderror
        </div>
        ...
        <div class="lj-fgroup" style="margin-bottom:14px;">
          ...
          <div style="font-size:.7rem;margin-top:4px;display:flex;align-items:center;justify-content:space-between;gap:8px;">
            <span style="color:var(--n400);">Min. 8 characters with letters and numbers</span>
            <span id="pwdStrengthLabel" style="font-weight:700;"></span>
          </div>
          @error('password')<div style="font-size:.72rem;color:var(--red);margin-top:3px;">{{ $message }}</div>@enderror
        </div>
        ...
        <div class="lj-fgroup">
          ...
          <div id="pwdMatchMsg" style="font-size:.72rem;margin-top:3px;display:none;"></div>
        </div>
  ...
  <div class="lj-card lj-section-gap">
    ...
    <form method="POST" action="{{ route('jobseeker.settings.notifications') }}">
      @csrf @method('PUT')
      <div style="padding:0 20px;">
        ...
        @endphp
        @foreach($notifSettings as [$key, $label, $sub])
...
<div class="lj-card lj-section-gap">
  ...
  <form method="POST" action="{{ route('jobseeker.settings.privacy') }}">
    @csrf @method('PUT')
    <div style="padding:0 20px;">
      @php
...
<div class="lj-card lj-section-gap">
  ...
  <div class="lj-card-body">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
      @php
        $infoCards = [
          ['Account Created', auth()->user()?->created_at->format('d M Y') ?? '—'],
          ['Last Login', auth()->user()?->last_login_at ? \Carbon\Carbon::parse(auth()->user()->last_login_at)->diffForHumans() : 'Just now'],
          ['Email', auth()->user()?->email ?? '—'],
          ['Account ID', '#'.auth()->user()?->id],
        ];
      @endphp
      @foreach($infoCards as [$label, $value])
        <div style="background:var(--n50);border:1.5px solid var(--n100);border-radius:9px;padding:13px 15px;">
          <div style="font-size:.68rem;font-weight:700;color:var(--n400);text-transform:uppercase;letter-spacing:.08em;margin-bottom:5px;">{{ $label }}</div>
          <div style="font-weight:600;color:var(--n800);font-size:.875rem;word-break:break-all;">{{ $value }}</div>
        </div>
      @endforeach
    </div>
  </div>
  ...
  <div class="lj-card-body">
    <div class="lj-danger-zone">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;">
        <div style="min-width:0;">
          <div style="font-weight:700;color:var(--red);font-size:.9rem;margin-bottom:5px;">Delete My Account</div>
          <div style="font-size:.81rem;color:#b91c1c;line-height:1.6;max-width:480px;">
            Once you delete your account, all your data — including your profile, resume, applications, and saved jobs — will be permanently removed.
...
<div id="deleteAccountModal" style="display:none;position:fixed;inset:0;background:rgba(15,23,42,.55);z-index:9999;align-items:center;justify-content:center;padding:20px;">
  <div style="background:#fff;border-radius:16px;padding:28px;max-width:400px;width:100%;box-shadow:var(--shadow-lg);">
    <div style="text-align:center;margin-bottom:20px;">
      <div style="width:54px;height:54px;border-radius:50%;background:var(--red-light);border:2px solid #fecaca;display:flex;align-items:center;justify-content:center;margin:0 auto 14px;font-size:1.3rem;color:var(--red);">
        <i class="fa-solid fa-trash"></i>
      </div>
      <div style="font-family:var(--f-display);font-size:1.1rem;font-weight:800;color:var(--n900);margin-bottom:5px;">Delete Account?</div>
      <div style="font-size:.83rem;color:var(--n600);line-height:1.6;">
        This will permanently delete all your data. Enter your password to confirm.
      </div>
    </div>
    <form method="POST" action="{{ route('jobseeker.settings.delete') }}">
      @csrf @method('DELETE')
      <div class="lj-fgroup" style="margin-bottom:16px;">
        <label class="lj-label">Your current password</label>
        <div class="lj-input-wrap">
          <i class="fa-solid fa-lock lj-input-ico"></i>
          <input class="lj-input has-ico" type="password" name="confirm_password" placeholder="Enter password" required/>
        </div>
      </div>
      <div style="display:flex;gap:10px;">
        <button type="button" onclick="closeDeleteModal()" class="lj-btn lj-btn-outline" style="flex:1;justify-content:center;">Cancel</button>
...
function checkPasswordMatch() {
  ...
  if (np === cp) { msg.textContent = '✓ Passwords match'; msg.style.color = 'var(--green)'; }
