{{-- ═══════════════════════════════════════════════════════
     resources/views/frontend/jobseeker/saved.blade.php
     Saved Jobs – LinearJobs Job Seeker Dashboard
═══════════════════════════════════════════════════════ --}}
...
@php
  ...
  $samples = [
    ['Senior PHP Developer','CoreDev Technologies','Chennai','₹45k–65k/mo','Full-time','3–5 Years','3 days ago','1'],
    ['Laravel + Vue Developer','PixelCraft Studios','Coimbatore','₹30k–50k/mo','Remote','2+ Years','5 days ago','2'],
    ['React.js Frontend Developer','Synapse Web','Madurai','₹25k–40k/mo','Full-time','1–2 Years','1 week ago','3'],
    ['Node.js Backend Developer','CloudSpark','Erode','₹35k–55k/mo','Hybrid','2–4 Years','2 weeks ago','4'],
    ['UI/UX Designer','Pixelcraft','Coimbatore','₹20k–35k/mo','Full-time','1–3 Years','3 weeks ago','5'],
  ];
@endphp
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:16px;">
      ...
      @foreach($savedJobs as $saved)
        <div class="lj-saved-job-card">
          <div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:12px;">
            <div style="width:40px;height:40px;border-radius:9px;background:var(--n100);border:1px solid var(--n200);display:flex;align-items:center;justify-content:center;color:var(--n500);font-size:.85rem;flex-shrink:0;overflow:hidden;">
            ...
            <div>
              ...
              <div class="lj-sjc-co">
                ...
                {{ $saved->job->company->name ?? $saved->job->company_name }}
                · {{ $saved->job->district }}
              </div>
            </div>
          </div>
          <div class="lj-sjc-tags">
            @if($saved->job->salary_range)
              <span class="lj-rec-tag salary">{{ $saved->job->salary_range }}</span>
            @endif
            @if($saved->job->job_type)
              <span class="lj-rec-tag type">{{ $saved->job->job_type }}</span>
            @endif
            @if($saved->job->experience_required)
              <span class="lj-rec-tag">{{ $saved->job->experience_required }}</span>
            @endif
          </div>
          @if($saved->job->expiry_date && \Carbon\Carbon::parse($saved->job->expiry_date)->isPast())
            <div style="font-size:.72rem;color:var(--red);margin:8px 0;display:flex;align-items:center;gap:4px;">
              <i class="fa-solid fa-triangle-exclamation"></i> This job has expired
            </div>
          @elseif($saved->job->expiry_date)
            <div style="font-size:.72rem;color:var(--orange);margin:8px 0;display:flex;align-items:center;gap:4px;">
              <i class="fa-solid fa-clock"></i> Closes {{ \Carbon\Carbon::parse($saved->job->expiry_date)->diffForHumans() }}
            </div>
          @endif
          <div class="lj-divider" style="margin:10px 0;"></div>
          <div class="lj-sjc-footer">
            <span style="font-size:.72rem;color:var(--n400);">Saved {{ $saved->created_at->diffForHumans() }}</span>
            <div style="display:flex;gap:7px;">
              <form method="POST" action="{{ route('jobseeker.dashboard.saved.remove', $saved->id) }}"
                onsubmit="return confirm('Remove from saved?')">
                @csrf @method('DELETE')
                <button type="submit" class="lj-btn lj-btn-danger lj-btn-sm" title="Remove">
    ...
    @else
      ...
      @foreach($samples as $s)
        <div class="lj-saved-job-card">
          <div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:12px;">
            <div style="width:40px;height:40px;border-radius:9px;background:var(--n100);border:1px solid var(--n200);display:flex;align-items:center;justify-content:center;color:var(--n500);font-size:.85rem;flex-shrink:0;">
              <i class="fa-solid fa-building"></i>
            </div>
            <div>
              <div class="lj-sjc-title">{{ $s[0] }}</div>
              <div class="lj-sjc-co"><i class="fa-solid fa-building" style="font-size:.65rem;"></i> {{ $s[1] }} · {{ $s[2] }}</div>
            </div>
          </div>
          <div class="lj-sjc-tags">
          ...
          <div class="lj-divider" style="margin:10px 0;"></div>
          <div class="lj-sjc-footer">
            <span style="font-size:.72rem;color:var(--n400);">Saved {{ $s[6] }}</span>
            <div style="display:flex;gap:7px;">
  ...
  @if($hasSaved && $savedJobs->hasPages())
    <div style="margin-top:20px;display:flex;justify-content:center;gap:6px;">
      ...
      <span style="align-self:center;font-size:.82rem;color:var(--n500);">
        Page {{ $savedJobs->currentPage() }} of {{ $savedJobs->lastPage() }}
      </span>
